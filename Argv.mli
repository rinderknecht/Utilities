...
(* The effect of the call [filter ~opt_with_arg ~opt_wo_arg] is a side
   ...
   arguments in [opt_with_arg] and [opt_wo_arg] remain. Consequently,
   the client is expected to make a copy of [Sys.argv] before calling
   [filter] and restore the original version if they want parse the
